...
import { FilterTasksWrapper } from "./filter-tasks-wrapper";
...
export default function AllTasks({
	...
	const isFiltering = hasActiveFilters("tasks");

	if (isLoading) return <AllTasksSkeleton />;

	return (
		<section className={cn("flex h-full w-full flex-col gap-4")}>
			<Header
				iconName={iconName}
				title={title}
	...
	return (
		<div className="flex flex-col gap-1">
			<div className="flex items-center gap-2 truncate">
				...
				/>
				<h2 className="truncate font-medium">{title || "No title"}</h2>
...
function TasksContent({
	...
	if (isFiltering) return <FilterTasksWrapper tasks={tasks} />;
